...
class PipelineConvertor(object):
    ...
    def convertOneShape(
        ...
        if len(data_type_list) < 2 or len(data_type_list) - 1 != len(
            self.convertor_list
        ):
        ...
        for i in range(len(self.convertor_list)):
            ...
            state = self.convertor_list[i].convertOneShape(
                rel_base_path, source_data_type, target_data_type
            )
    ...
    def convertAll(
        ...
        for root, dirs, files in os.walk(
            self.convertor_list[0].source_root_folder_path
        ):
            if data_type_list[0] == "/":
                if dirs:
                    continue
                ...
                rel_base_path = os.path.relpath(
                    root, self.convertor_list[0].source_root_folder_path
                )
            ...
            for file in files:
                ...
                rel_base_path = os.path.relpath(
                    root + "/" + file, self.convertor_list[0].source_root_folder_path
                )
